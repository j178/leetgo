...
func RunTestsWithString(t *testing.T, f interface{}, testcases string, targetCaseNum int) (err error) {
    lines := splitAndRemoveEmptyLine(testcases)
    n := len(lines)
    if n == 0 {
    ...
    if n%tcSize != 0 {
        return fmt.Errorf("invalid testcases: 有效行数 %d, 应该是 %d 的倍数", n, tcSize)
    }
...
func RunClassTestsWithString(t *testing.T, constructor interface{}, testcases string, targetCaseNum int) error {
    ...
    // 每三行一组数据
    if n%3 != 0 {
        return fmt.Errorf("invalid testcases: 有效行数 %d，应该是 3 的倍数", n)
    }
