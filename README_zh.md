...
搜索结果随输入自动更新，按 `Enter` 或 `Esc` 离开输入框并浏览结果。
点击顶部筛选按钮，或按 `d`/`s`/`t` 打开下拉菜单：难度和状态点选即生效，标签支持搜索和多选，并提供 `Apply`/`Cancel` 按钮。按 `Esc` 或点击菜单外部可关闭菜单并放弃未应用的修改。
终端宽度至少为 80 列时，右侧会显示高亮题目的 Markdown 预览。用 `Tab` 切换区域，或用 `←`/`→` 聚焦列表或预览，再用方向键、`PgUp`/`PgDn` 操作当前区域；`J`/`K` 或 `Ctrl+D`/`Ctrl+U` 可随时滚动预览。
点击题目可切换预览，点击标签可切换勾选状态；滚轮操作鼠标所在的列表或正文区域，按 `Enter` 确认选择。
